...
" Last Modifier: Sarrow (Sarrow104 at gmail.com)
" Last Change: 2009-07-21
" Version:     1.5.0
"=============================================================================
" See documentation in accompanying help file
" You may use this code in whatever way you see fit.
"
" Inprovement By Sarrow:
"    1. correct some bug, like:
"
"       path[0] == '\\' => path[0] == '\'
"
"    2. support project files in current directory;
"       so if you can place your project file (.vimprj) with your source
"       files. then move them what ever directory as you wish
"
"    3. split the syntax highlighting part to $vimfiles/syntax/vimprj.vim
"
"    4. now, you can unload project file much easy with:
"
"       :ProjectClose
"
"       if you have already loaded a project file with:
"
"       :Project [empty | path_to_your_.vimprj_file]
"
"	or
"
"	:e path_to_your_.vimprj_file | Load
"
"    5. when your already have an .vimprj file in certain directory,
"       it was boring if you use <LocalLeader>C to create the project entry!
"       now, you can use:
"
"       <LocalLeader>A
"
"       to make entries automatically
"
"       NOTE: the entry name is the name of the directory where you place your
"       .vimprj file.
"
"       then, you can modify the entrise manually. After that, use <LocalLeader>R to
"       refresh their contents.
"
"  TODO
"
"    1. auto-complete support, like
"
"	entry_name=<C-X><C-U>
"
"	CD=<C-X><C-U>
...
"	...
"
"    2. template project support, like
"
"       C/Cpp project
"
...
function! s:Project(filename) " <<<
    " Initialize <<<
    " If: invoke with ":Project %"
    if &filetype == 'vimprj' && a:filename == expand('%')
	bdelete
    endif
    " Sarrow: 2009-07-20
    function! s:ProjectClose()
	if !exists('g:proj_running')
	    return
	endif
	exec 'silent ' . bufwinnr(g:proj_running) . 'wincmd w'
	update|bwipeout|delcommand ProjectClose
	command! -nargs=? -complete=file Project call <SID>Project('<args>')
    endfunction
    " End:
    if exists("g:proj_running") && strlen(a:filename)
	call s:ProjectClose()
    endif
    ...
    "   Returns
    "       2   remote file
    "       1   disc file with absolute path.
    "       0   relative path
	...
	"Sarrow:
	"echoe '('. string(lineno) .'|'. parent_home .'|'. parent_c_d .')'
	"End:
        ...
        if !strlen(filter) | let filter = parent_filter | endif
    ...
    function! s:OpenEntry(line, precmd, editcmd, dir)
		...
		" Sarrow:
		" New:
		" if matchstr(getline(a:line), '\ze\S.\{-}\ze\(\|\s*#.*\)$') != fname
		"     echo a:line .'|'. fname
		" endif
		" End:
                if strlen(fname) == 0
    ...
    " Sarrow:2009-07-20
    " TODO
    " s:AutoCreatePrjEntry(recursive) <<<
    "   when new a .vimprj file ...
    function! s:AutoCreatePrjEntry(recursive)
        let fname = expand('%:t')
        if &filetype != 'vimprj'
	    echoe 'Error: expecting vimprj filetype!'
            return
        endif
        let line=line(1)
        " TODO check: is it an empty file?
        let parent_dir = expand('%:p:h:t')
        let entry_name = strlen(parent_dir) ? parent_dir : inputdialog('Enter the Name of the Entry: ')
		...
		" New:
		" let dir=matchstr(dir, '\C'.glob(home))
		" End:
            endif
        endif
		...
		call mkdir(home.dir, 'p')
                ...
                echoe 'Error: cannot crate dir `' . home . dir . '`'
                return
	    endif
        endif
        let c_d = '.'
        ...
        " Do the work
        call s:DoEntryFromDir(a:recursive, line, entry_name, home.dir, '.', c_d, filter_directive, filter, foldlev, 0)
    endfunction ">>>
    " End:
    ...
    function! s:RefreshEntriesFromDir(recursive)
        ...
        if just_a_fold == 0
            ...
            "endif
            let name = matchstr(infoline, '^[#\t ]*\zs[^=]*\ze=.*')
            if !strlen(name)
                return                  " If there's no name, we're done.
            endif
            if (home == '') || (name == '')
        ...
        if just_a_fold == 0
            ...
            if !isdirectory(glob(home))
                call confirm('2cd "'.home.'" is not a valid directory.', "&OK", 1)
            else
	...
	"formate: "Directory=home CD=workdirectory"
        " Thanks to Adam Montague for pointing out the need for @ in urls.
        let home=matchstr(a:info, '^[^=]*=\zs\(\\ \|\f\|:\|@\)\+\ze.*')
	" quoted home string
        if home == ''
	    let home=matchstr(a:info, '.\{-}"\zs.\{-}\ze".*')
            if home != ''
        ...
        " Sarrow: 2009-07-20
	" echoe 'info|home:(' . a:info .'|' . home .')'
        " End:
        return home
    endfunction
    function! s:GetFilter(info, parent_filter)
        let filter = matchstr(a:info, '.*\<filter="\zs[^"]*\ze.*')
        if filter == ''
	    let filter = a:parent_filter
	endif
        return filter
    endfunction
    function! s:GetCd(info, home)
        let c_d = matchstr(a:info, '.*\<CD=\zs\(\\ \|\f\|:\)\+\ze.*')
        if c_d == ''
            let c_d = matchstr(a:info, '.*\<CD="\zs.\{-}\ze".*')
            if c_d != ''
		let c_d = escape(c_d, ' ')
	    endif
        endif
        ...
        elseif c_d != '' && !s:IsAbsolutePath(c_d)
            if a:home != ''
                let c_d = a:home.'/'.c_d
            endif
        endif
    ...
    if !exists("g:proj_running")
        ...
        " Mappings <<<
	" Sarrow: 2009-07-08
	" nnoremap <buffer> <silent> J 		/}/-1<CR>
	" nnoremap <buffer> <silent> K 		?{?+1<CR>
	" nnoremap <buffer> <silent> N 		$/{/0<CR>
	" nnoremap <buffer> <silent> P 		?{?0<CR>
	nnoremap <buffer> <silent> J 		Vi{^
	nnoremap <buffer> <silent> K 		Vi{o^
	nnoremap <buffer> <silent> N 		Va{^
	nnoremap <buffer> <silent> P 		Va{o^
	" FIXME
        " Sarrow: 2009-07-26
	" 注意， "N" 和 "p" 操作，当已经在最上层的时候，它们会无法选中
	" 区域，并一直处于 "行选择" 模式。
	" 解决方案1：
	"     利用 search() 函数来搜索位置，并用定点光标的函数来选中。
	" 解决方案2：
	"     利用 本脚本以及实现的函数来定位。
	" End:2009-07-08
	...
	nnoremap <buffer> <silent> <LocalLeader>a :call <SID>AutoCreatePrjEntry(0)<CR>
	nnoremap <buffer> <silent> <LocalLeader>A :call <SID>AutoCreatePrjEntry(1)<CR>
	...
	command -nargs=0        ProjectClose call <SID>ProjectClose()
    endif
    " End:
endfunction " >>>
